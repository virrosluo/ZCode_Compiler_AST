...
def parse_functions(code: str) -> dict:
    ...
    function_pattern = r'def\s+([a-zA-Z_]\w*)\s*\(([^)]*)\)\s*:|input\s*=\s*"""(.*?)"""|expect\s*=\s"""*(.*?)"""|self.assertTrue\s*\(\s*TestAST.test\s*\(\s*input\s*,\s*expect\s*,\s*(.*?)\)\s*\)'
...
\t\tself.assertTrue(TestAST.test(input,expect,{int(fn_info['tc_num']) + 200}))"""
    return output